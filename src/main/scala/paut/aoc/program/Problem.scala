...
import Console.*
import paut.aoc.action.Results
import java.time.LocalDate
import paut.aoc.Files
...
abstract class Problem[A]
  ...
  private def writeResult(eval: TimedEval[A])(implicit printresult: Boolean): Unit = {
    val newRes = Result(
            ...
            day = day,
            part = part,
            solution = eval.result.toString,
            time = eval.duration,
            timestamp = java.time.LocalDateTime.now(),
            submitted = false
          )

    def replace(result: Result) = {
      os.write.over(
        Files.resultsFile,
        os.read(Files.resultsFile).replace(result.raw, newRes.raw)
      )
    }
    ...
    Results.GetOne(part, date).get match {
      case None => add(newRes)
      case Some(result) => 
